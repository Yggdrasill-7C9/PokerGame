...
+ (UIImage*)creatBackground:(CGRect)rect fillColor:(UIColor*)fillColor
{
    ...
    CGContextSetFillColorWithColor(context, [JKUtil getColor:@"c5c5c5"].CGColor);
...
+ (void)preservationSearchText:(NSString *)searchText{
    ...
    NSArray *arr = [userDefaults arrayForKey:SEARCHPRICE];
    if (!arr) {
        [[NSUserDefaults standardUserDefaults] setObject:@[searchText] forKey:SEARCHPRICE];
    }else{
        ...
        [mutableArr insertObject:searchText atIndex:0];
        if (mutableArr.count >= HISTORY_SEARCHCOUNT) {
            NSArray *smallArray = [mutableArr subarrayWithRange:NSMakeRange(0, HISTORY_SEARCHCOUNT)];
            [[NSUserDefaults standardUserDefaults] setObject:smallArray forKey:SEARCHPRICE];
        }else{
            [[NSUserDefaults standardUserDefaults] setObject:mutableArr forKey:SEARCHPRICE];
